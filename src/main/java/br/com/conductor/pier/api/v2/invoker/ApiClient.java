...
import br.com.conductor.pier.api.v2.invoker.auth.ApiKeyAuth;
import br.com.conductor.pier.api.v2.invoker.auth.Authentication;
import br.com.conductor.pier.api.v2.invoker.auth.HttpBasicAuth;
import br.com.conductor.pier.api.v2.invoker.auth.OAuth;
...
public class ApiClient {
     ...
     private GerenciadorRSA gerenciadorRSA;

     private GerenciadorAES gerenciadorAES;

     public ApiClient() {

          if (Configuracoes.CRITOGRAFAR) {

               // RSA --------------------
               gerenciadorRSA = new GerenciadorRSA();
               privateKey = gerenciadorRSA.carregaChavePrivada(Configuracoes.PRIVATE_KEY_EMISSOR);
               publicKey = gerenciadorRSA.carregaChavePublica(Configuracoes.PUBLIC_KEY_CDT);

               // Decodificando a chave AES
               byte[] secretKeyTemp = Base64.getDecoder().decode(Configuracoes.SECRET_KEY_EMISSOR);
               byte[] secretKeyByte = gerenciadorRSA.descriptografar(secretKeyTemp, privateKey);

               // Carrega chave AES
               gerenciadorAES = new GerenciadorAES();
               secretKey = gerenciadorAES.carregaChaveAES(secretKeyByte);
     ...
     public SecretKey getSecurytKey() {
          
          return secretKey;
          
     }
     
     public GerenciadorAES getEngineEAS() {
          
          return gerenciadorAES;
      ...
      * <p>
      * Note: If you make changes to the object mapper, remember to set it back via <code>setObjectMapper</code> in order to trigger HTTP client rebuilding.
      * </p>
      */
     ...
     public <T> T invokeAPI(String path, String method, List<Pair> queryParams, Object body, Map<String, String> headerParams, Map<String, Object> formParams, String accept, String contentType, String[] authNames, GenericType<T> returnType) throws ApiException {
          ...
          if (body != null) {
               ...
               try {
                    ...
                    if (Configuracoes.CRITOGRAFAR) {
                         System.out.println("------------------------ENVIO CRIPTOGRAFADO------------------------");
                         byte[] objetoCriptografado = gerenciadorAES.criptografar(json.getBytes(), secretKey);
                         bodyCriptografado = Base64.getEncoder().encodeToString(objetoCriptografado);

                    } else {

                         bodyCriptografado = json;
          ...
          ClientResponse response = getAPIResponse(path, method, queryParams, bodyCriptografado, headerParams, formParams, accept, contentType, authNames);
          ...
          if (response.getStatusInfo() == ClientResponse.Status.NO_CONTENT) {
               ...
               if (returnType == null) {
                    ...
                    String responseEncriptado = response.getEntity(stringType);

                    byte[] dadosDescriptografados = null;

                    if (org.apache.commons.codec.binary.Base64.isBase64(responseEncriptado)) {

                         if (Configuracoes.CRITOGRAFAR) {
                              System.out.println("------------------------RESPOSTA CRIPTOGRAFADA------------------------");
                              byte[] dadosDecodeBase64 = Base64.getDecoder().decode(responseEncriptado);
                              dadosDescriptografados = gerenciadorAES.decriptografar(dadosDecodeBase64, secretKey);

                         } else {

                              dadosDescriptografados = responseEncriptado.getBytes();
                    ...
                    if (dadosDescriptografados != null) {
                         ...
                         try {
                              ...
                              return objectMapper.readValue(dadosDescriptografados, returnType.getRawClass());
                    ...
                    try {
                         ...
                         if (returnType.getRawClass().isArray()) {

                              return response.getEntity(returnType);

                         } else {

                              return objectMapper.readValue(responseEncriptado, returnType.getRawClass());
